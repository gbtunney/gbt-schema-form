...
Proposals are **ephemeral**. They can be regenerated anytime. There is no `ProposalRun` entity — proposals are returned directly from the API and never persisted.

## FieldProposal

A proposal suggests a value for a specific JSON pointer path. It is tied directly to an evidence item (no intermediate run object).
...
Note: `value` is used (not `valueJson`). There is no `valid` flag or `runId` — validation happens in the UI via JSON Schema.
