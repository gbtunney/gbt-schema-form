...
# Usage: GH_TOKEN=<token> bash scripts/create-github-issues.sh
#
...
echo ""
echo "==> Looking for project '${PROJECT_TITLE}'…"
PROJECT_ID=$(gh project list --owner gbtunney --format json \
    --jq ".projects[] | select(.title == \"${PROJECT_TITLE}\") | .number" 2>/dev/null || true)

if [ -z "$PROJECT_ID" ]; then
    echo "  WARNING: project '${PROJECT_TITLE}' not found - issues will be created without project assignment."
fi
...
if [ -n "$PROJECT_ID" ]; then
    echo ""
    echo "==> Setting project fields (Status=Inbox, Priority=Medium)…"
    echo "  NOTE: field values require project field IDs - update the FIELD_ID/OPTION_ID values below"
    echo "  Run: gh project field-list \$PROJECT_ID --owner gbtunney --format json"
    echo "  Then set: gh project item-edit --project-id \$PROJECT_ID --id \$ITEM_ID --field-id \$FIELD_ID --single-select-option-id \$OPTION_ID"
fi
